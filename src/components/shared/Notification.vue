  ...
  >
    <div class="notif__row">
      ...
      <div class="notif__meta">
        ...
        <span v-if="!notification.read" class="notif__dot" :aria-label="$t('غير مقروء')"></span>
        <DsButton
...
const emit = defineEmits<{
  (e: 'read', val: AppNotification): void
  (e: 'click', val: { event: Event; notification: AppNotification }): void
  (e: 'action-click', val: { event: Event; notification: AppNotification; to: string | undefined }): void
}>()

const resolvedType = computed<NotifType>(() => {
  const t = props.notification?.type ?? 'info'
  return (['info', 'success', 'warning', 'danger'] as NotifType[]).includes(t as NotifType)
    ? (t as NotifType)
    : 'info'
})

const iconPath = computed(() => ICONS[resolvedType.value] ?? ICONS.info)

const relativeTime = computed(() => {
  const ts = props.notification?.timestamp
  if (!ts) return ''
...
const ariaLabel = computed(() => {
  const parts: string[] = []
  if (!props.notification.read) parts.push('غير مقروء')
  if (props.notification.title) parts.push(props.notification.title)
...
function onCardClick (e: Event): void {
  ...
  emit('click', { event: e, notification: props.notification })
}

function onActionClick (e: Event): void {
  emit('action-click', {
    event: e,
    notification: props.notification,
    to: props.notification.actionTo
  })
}
</script>
